...
if input_method == "会社名で検索":
    ...
    if search_text and stock_list_loaded:
        matches = stock_list_df[stock_list_df["銘柄名"].str.contains(search_text, case=False, na=False)]
    ...
    ticker_code = st.sidebar.text_input("証券コード（例：7203）")
    if ticker_code and stock_list_loaded:
        match = stock_list_df[stock_list_df["コード"] == ticker_code.strip()]
        if not match.empty:
